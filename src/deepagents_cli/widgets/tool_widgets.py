...
class EditFileApprovalWidget(ToolApprovalWidget):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(f"[#f4f8fc]File:[/#f4f8fc] {file_path}  {stats_str}")
        yield Static("")
    ...
    def _format_stats(self, additions: int, deletions: int) -> str:
        ...
        parts = []
        if additions:
            parts.append(f"[#72d69f]+{additions}[/#72d69f]")
        if deletions:
            parts.append(f"[#ff7a7a]-{deletions}[/#ff7a7a]")
        return " ".join(parts)
    ...
    def _render_strings_only(self, old_string: str, new_string: str) -> ComposeResult:
        ...
        if old_string:
            yield Static("[#b8c6d4]Removing:[/#b8c6d4]")
            yield from self._render_string_lines(old_string, is_addition=False)
            yield Static("")

        if new_string:
            yield Static("[#b8c6d4]Adding:[/#b8c6d4]")
    ...
    def _render_diff_line(self, line: str) -> Static | None:
        ...
        if line.startswith("-"):
            return Static(f"[#ff7a7a]- {content}[/#ff7a7a]")
        if line.startswith("+"):
            return Static(f"[#72d69f]+ {content}[/#72d69f]")
        if line.startswith(" "):
            return Static(f"[#9aa8b7]  {content}[/#9aa8b7]")
        if line.strip():
    ...
    def _render_string_lines(self, text: str, *, is_addition: bool) -> ComposeResult:
        ...
        lines = text.split("\n")
        color = "#72d69f" if is_addition else "#ff7a7a"
        ...
        if len(lines) > _MAX_PREVIEW_LINES:
            ...
            yield Static(f"[#9aa8b7]... ({remaining} more lines)[/#9aa8b7]")


class FastApplyApprovalWidget(ToolApprovalWidget):
    """Approval widget for fast_apply - shows instruction and code edit snippet."""
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(f"[#f4f8fc]File:[/#f4f8fc] {file_path}")
        yield Static(f"[#b8c6d4]Model:[/#b8c6d4] {model}")
        if instruction:
            yield Static("")
            yield Static("[bold]Instruction:[/bold]")
            yield Static(_escape_markup(instruction), markup=True, classes="approval-description")

        if code_edit:
            yield Static("")
            yield Static("[bold]Code Edit:[/bold]")
